                      ...
                      <tr className={`transition-colors ${r.decision === "REJECTED" ? "bg-red-50/40" : r.decision === "APPROVED" ? "bg-emerald-50/30" : "hover:bg-seekop-50/30"}`}>
                        ...
                        <td className="py-2 pr-3">
                          <button onClick={() => onViewReceipt(r)} className="text-left hover:underline">
                            ...
                            {r.isCfdi && <span className="ml-1 text-[9px] text-blue-700 bg-blue-50 px-1 py-0.5 rounded border border-blue-200">CFDI</span>}
                            {r.rfcEmisor && <span className="ml-1 text-[9px] text-gray-500 font-mono">{r.rfcEmisor}</span>}
                          </button>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-left text-gray-400 border-b border-gray-100">
                    ...
                    <th className="py-1.5 pr-3 font-medium text-center">Cant.</th>
                    <th className="py-1.5 pr-3 font-medium text-right">P. Unit.</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {receipt.lineItems.map((item, idx) => (
                    <tr key={idx} className="text-gray-700">
                      ...
                      <td className="py-1.5 pr-3 text-center">{item.quantity ?? "—"}{item.unit ? ` ${item.unit}` : ""}</td>
                      <td className="py-1.5 pr-3 text-right">{item.unit_price != null ? `$${Number(item.unit_price).toFixed(2)}` : "—"}</td>
